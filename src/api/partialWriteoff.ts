...
export async function openPartialOperation(budgetId: string): Promise<PartialWriteoffOperation> {
  const data = await invoke<{ operation: PartialWriteoffOperation }>({ action: 'open_operation', budget_id: budgetId });
