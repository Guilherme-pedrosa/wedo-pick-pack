BEGIN;
CREATE OR REPLACE FUNCTION public.compras_partial_revision() RETURNS text
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT md5(coalesce(jsonb_agg(to_jsonb(t) ORDER BY t.id)::text,'[]')) FROM (
    SELECT o.id,o.version,o.status,(SELECT jsonb_agg(jsonb_build_array(i.id,i.original_quantity,i.withdrawn_quantity,i.reserved_quantity) ORDER BY i.id)
      FROM partial_writeoff_items i WHERE i.operation_id=o.id) items
    FROM partial_writeoff_operations o WHERE o.status NOT IN ('completed','cancelled')
  ) t;
$$;
REVOKE ALL ON FUNCTION public.compras_partial_revision() FROM PUBLIC;

-- A rotina só lê saldos e grava o resultado da lista. Nenhum comando de estoque/Auvo.
CREATE OR REPLACE FUNCTION public.compras_worker_api(p_token text,p_action text,p_payload jsonb DEFAULT '{}')
...
BEGIN
  ...
  ELSIF p_action='save' THEN
    ...
    IF coalesce((result->>'purchaseScanVersion')::int,0)<>2 OR jsonb_typeof(result->'itensList') IS DISTINCT FROM 'array'
    ...
    VALUES(jsonb_array_length(result->'itensList'),jsonb_array_length(result->'itensOkList'),jsonb_array_length(result->'itensCobertosporPedido'),
      ...
      result->'itensList',coalesce(p_payload->'config','{}')||jsonb_build_object('purchase_scan_version',2,
...
END $$;
REVOKE ALL ON FUNCTION public.compras_worker_api(text,text,jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.compras_worker_api(text,text,jsonb) TO anon,authenticated,service_role;
COMMIT;
