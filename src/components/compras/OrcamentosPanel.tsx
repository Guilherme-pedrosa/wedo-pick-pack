...
import { getStatusOrcamentos, listOrcamentos, buildListaCompras } from '@/api/compras';
...
import { FileText, RefreshCw, ShoppingCart, Loader2 } from 'lucide-react';
...
export default function OrcamentosPanel() {
  ...
  // Fetch orcamentos when selected situacoes change
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await buildListaCompras(selectedSituacoes, (step, checked, total) => {
        setProgress({ step, checked, total });
      });
      setResult(result);
      toast.success(
        `Lista gerada! ${result.totalProdutosSemEstoque} itens para comprar, ${result.totalProdutosOk} com estoque.`
      );
    } catch (err) {
  ...
  const totalProdutos = (orc: GCOrcamento) => orc.produtos?.length || 0;
      ...
      <div className="sticky top-0 bg-card z-10 p-3 space-y-3 border-b border-border">
        ...
        {statusQuery.isLoading ? (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Loader2 className="h-3 w-3 animate-spin" /> Carregando situações…
          </div>
        ) : (
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {selectedSituacoes.length === 0 && (
          <p className="text-center text-xs text-muted-foreground py-6">
            Selecione ao menos uma situação acima
          </p>
        )}
        {loadingOrc && (
        ...
        {orcamentos.map(orc => (
          <Card key={orc.id} className="p-3 border-l-4 border-l-green-500">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
              ...
              <span>·</span>
              <span>{totalProdutos(orc)} itens</span>
            </div>
          </Card>
        ))}
      </div>
      ...
      <div className="sticky bottom-0 bg-card border-t border-border p-3 space-y-2">
        ...
        {orcamentos.length > 0 && (
          <p className="text-xs text-center text-muted-foreground">
            {orcamentos.length} orçamento(s) selecionado(s)
          </p>
        )}
      </div>
    </div>
