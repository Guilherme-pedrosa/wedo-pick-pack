BEGIN;
-- Consulta leve de atualidade para recuperar janelas atrasadas sem varrer o GC em toda execução.
...
BEGIN
  ...
  IF p_action='status' THEN
    RETURN jsonb_build_object('last_success_at',(SELECT max(created_at) FROM compras_snapshots WHERE status='success'));
  ELSIF p_action='partials' THEN
